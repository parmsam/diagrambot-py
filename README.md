...
diagrambot is an interactive Shiny app for creating Mermaid and Graphviz diagrams using AI. It helps you generate flowcharts, sequence diagrams, network graphs, organizational charts, and more through both voice commands (using GPT-4o Realtime) and text chat interfaces. diagrambot supports both Mermaid syntax for a wide variety of structured diagrams and Graphviz/DOT syntax for complex network and hierarchical visualizations. diagrambot is a Python rewrite of the [original diagrambot](https://github.com/parmsam/diagrambot) R package, which is a fork of [ggbot2](https://github.com/tidyverse/ggbot2/) from [Posit](https://posit.co/).
...
Once you have an API key, you will need to set it as an environment variable named `OPENAI_API_KEY`. You can do this by adding the following line to a `.env` file in your project directory:
...
You can install diagrambot directly from GitHub using:
...
To launch diagrambot with voice control, run:

```python
from diagrambot import diagrambot_voice
...
For a text-based chat interface, use:

```python
from diagrambot import diagrambot_chat
...
```bash
python -m diagrambot.chat
```

This interface uses `shinychat` to provide a traditional text chat experience. It's great for when you want to type your requests or when voice isn't convenient.
